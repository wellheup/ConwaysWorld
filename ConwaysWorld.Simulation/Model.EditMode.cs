...
public partial class Model
{
        ...
        public void MoveCell(int fromCol, int fromRow, int toCol, int toRow)
        {
                ...
                if (!src.IsAlive) return;
                CellGrid[fromCol, fromRow] = new Cell_Basic(fromCol, fromRow, false);
                // Zombie cells carry a Necromancer reference; move the object itself rather than recreating.
                if (src is Cell_Zombie zombie)
                {
                        zombie.Column = toCol;
                        zombie.Row = toRow;
                        CellGrid[toCol, toRow] = zombie;
                }
                else
                {
