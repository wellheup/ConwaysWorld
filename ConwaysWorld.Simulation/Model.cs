		...
		for (int c = 0; c < _columns; c++)
		{
			for (int r = 0; r < _rows; r++)
			{
				...
				if (wasAlive)
				{
					...
					// Irradiated cells are alive but do NOT count toward the living population
					// (so a grid of only Irradiated cells still reads as "all dead").
					if (CellGrid[c, r].CellType != CellType.Irradiated)
						_currentPopulation++;
				}
				else
				{
