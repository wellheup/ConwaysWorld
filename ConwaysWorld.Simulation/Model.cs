...
public partial class Model
{
	...
	{
		// 1. Kill zombies of any newly-dead Necromancers.
		var dead = _activeNecromancers.Where(n => !n.IsAlive).ToList();
		foreach (var necro in dead)
		{
			necro.KillAllZombies(CellGrid);
			_activeNecromancers.Remove(necro);
		...
		for (int c = 0; c < _columns; c++)
			for (int r = 0; r < _rows; r++)
			{
						...
						&& !_activeNecromancers.Contains(necro))
				{
					...
					// The initial burst of 3 resurrections is handled by
					// UpdateSpecialActions (which calls SpecialActions on all cells).
		...
		for (int c = 0; c < _columns; c++)
			...
			{
				var cell = CellGrid[c, r];
				// A zombie slot is one that used to be a zombie but is now dead.
