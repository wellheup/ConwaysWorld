...
public partial class Model
{
	...
	public void UpdateCellConditions()
	{
		...
		for (int c = 0; c < _columns; c++)
		{
			for (int r = 0; r < _rows; r++)
			{
				...
				// Irradiated cells are permanent — no conditions, no conversion, no nation-join.
				...
				bool isUndeadType = cell.CellType == CellType.Zombie || cell.CellType == CellType.Necromancer;
				...
				if (cell.IsAlive && cell.Age >= 1 && cell.Nationality < 0 && !hasCooldown && !isNationlessType)
				{
					// Reuse scratch list — avoids a new List<int> allocation per cell.
	...
	{
		// Snapshot king/count per nation before Census runs — avoids two ToDictionary() allocs.
		// We store snapshot data directly on the nation objects to skip the temp dict entirely.
		foreach (var nation in Nations.Values)
		{
		...
		// King max-age and population-growth revolutionary triggers.
		// ToList() snapshot needed here only — avoids mutation-during-iteration if a
		// TrySpawnRevolutionary adds a nation.  Nations dict is small so cost is low.
		foreach (var nation in Nations.Values.ToList())
		{
			// Age out kings who have reigned for Cell_King.MaxAge steps.
			...
			// Spawn a revolutionary when population grows for half a king's lifespan consecutively.
	...
	/// <summary>
	/// Maintains <see cref="_activeNecromancers"/>:
	/// <list type="bullet">
	///   <item>Discovers newly spawned Necromancers on the grid and registers them.</item>
	///   <item>When a tracked Necromancer has just died, kills all its zombies and permanently
	///   clears their <see cref="Cell.LastType"/> so they cannot be re-resurrected.</item>
	///   <item>Also ensures any cell a Warrior/Hunter kills that is a Zombie has its
	///   slot permanently destroyed (no LastType).</item>
	/// </list>
	/// </summary>
	private void UpdateNecromancers()
	{
		// 1. Kill zombies of any newly-dead Necromancers (no LINQ — avoids allocation).
		...
		// 2. Scan for new Necromancers not yet tracked.
		//    Only scan if random life is active (necromancers can only appear via spawning).
		for (int c = 0; c < _columns; c++)
			for (int r = 0; r < _rows; r++)
			{
		...
		// Track whether any necromancer has ever created zombies.
		...
		// 3. Ensure any Zombie slot that is now dead (killed by Warrior/Hunter/etc.)
		//    has its LastType cleared permanently so it cannot be re-raised.
		//    Skip entirely if no zombie has ever existed — avoids an O(N) scan every step.
	...
	/// <summary>
	/// Collects the set of active (attackerNation, targetNation) pairs from all live
	/// <see cref="Cell_Soldier"/> cells on the grid.
	/// </summary>
	...
	/// <summary>
	/// Detects when an attack wave ends (a previously-active soldier pair has no live
	/// soldiers remaining) and conditionally merges the defeated nation into the victor.
	/// A merge fires when the defender's population is below 75 % of the attacker's.
	/// Called once per step, immediately after <see cref="UpdateSpecialActions"/>.
	/// </summary>
	...
	/// kills the loser's King without succession, re-tags all loser cells as the winner,
	/// removes the loser from <see cref="Nations"/>, and fires a <c>kingdom_destroyed</c> event.
	/// </summary>
	private void MergeNationInto(int loserNation, int winnerNation)
	{
		...
		// Kill the king immediately — no succession.
		...
		// Re-assign all surviving loser citizens to the winner.
		for (int c = 0; c < _columns; c++)
			for (int r = 0; r < _rows; r++)
			{
				var cell = CellGrid[c, r];
	...
	/// <summary>
	/// Promotes a random eligible citizen of <paramref name="nation"/> to
	/// <see cref="CellType.Revolutionary"/>, founding a new nation if a slot is available.
	/// Eligible candidates exclude Kings, Warriors, Diplomats, Rebels, and existing Revolutionaries.
	/// Does nothing if no candidates exist.
	/// </summary>
